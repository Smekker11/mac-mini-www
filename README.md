...
- Edit the services params `services.ini` to customize.
...
- Clicking "Download All JARs" hits the handler at the top of `minecraft/index.php`. It creates a temporary ZIP in the system temp dir named like `jars_YYYYmmdd_HHMMSS_######.zip`, streams it to the requester, then deletes the temp file.
...
- `services.php` and `services.ini` — simple service checks / config.
